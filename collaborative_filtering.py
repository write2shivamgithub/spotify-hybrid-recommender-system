...
# set paths
# outputs
track_ids_save_path = "data/track_ids.npy" # unique track_id numpy
filtered_data_save_path = 'data/collab_filtered_data.csv'  # details 30k songs
interaction_matrix_save_path = 'data/interaction_matrix.npz'  # 30 MB size(sparse matrix )

# input paths
songs_data_path = 'data/cleaned_data.csv'
user_listening_history_data_path = 'data/User Listening History.csv'  # This data will be read through dask
...
def collaborative_recommendation(song_name, artist_name, track_ids, songs_data, interaction_matrix, k=5):
    ...
    ind = np.where(track_ids['track_id'] == input_track_id)[0].item()
